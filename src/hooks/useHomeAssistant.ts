...
export function useHomeAssistant() {
    ...
    const { pushHumidity, getSlice } = useHistoryBuffer();
    ...
    const applyEntityState = useCallback(
        (entityId: string, stateValue: string) => {
            ...
            if (entityId === entities.window) {
                setState((prev) => ({
                    ...prev,
                    windowOpen: stateValue === "on",
                    lastUpdate: new Date(),
                }));
                ...
                setState((prev) => ({
                    ...prev,
                    pumpOn: stateValue === "on",
                    lastUpdate: new Date(),
                }));
            } else if (entityId === entities.light) {
                setState((prev) => ({
                    ...prev,
                    lightOn: stateValue === "on",
                    lastUpdate: new Date(),
                }));
            } else if (entityId === entities.humidity) {
                ...
                if (!isNaN(val)) {
                    ...
                    setState((prev) => ({
                        ...
                        history: {
                            ...prev.history,
                            humidity: getSlice(7 * 24 * 60 * 60 * 1000)
                                .humidity,
                        },
                    }));
                }
